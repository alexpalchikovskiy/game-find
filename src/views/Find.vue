<template>
    <div 
        :class="'game is-'+$route.params.type+' has-winner-'+winner+' is-status-'+status" 
    ...
    >
        ...
        <div class="items">
            ...
            <div 
                ...
                :key="index"
                v-on:click="touch(index)"
            >
                <div 
                    class="game-cover" 
                    :class="{'is-opened': i.opened}" 
                    :style="'background-image: url(/img/'+$route.params.type+'-cover.png);'"
                ></div>
                <div 
                    v-if="i.item" 
                    :class="'game__x game__x-'+index" 
                    :style="'background-image: url(/img/'+$route.params.type+'-item.png);'"
                ></div>
            </div>
        </div>
    ...
    methods: {
        start: function(){
            ...
            this.list.map( (i, index) => {
                if( i.item ){
                    let b = document.querySelector(`.game__x-${index}`);
                    let rect = b.getBoundingClientRect();
                    let delta = i.id * (rect.width + 4);
                    i.tween = gsap.to(b, {x: -1*(rect.x - 20) + delta, y: -1*(rect.y - 20), duration: 0, ease: "power1.inOut"});
                    i.tween.play();
                    setTimeout( () => {
                        i.tween.totalDuration( 1 );
                        i.tween.reverse(0);
                    }, 300 );
                }
            } );
            setTimeout( () => { this.status = 'playing'; }, 2300 );
        },
        getArray: function(){
            let a = [];
            for( let x = 0; x < 45; x++ ){
        ...
        touch: function( index ){
            if( this.list[ index ].opened ){
                return false;
            }
            this.counter.touches++;
            this.list[ index ].opened = true;
            if( this.list[ index ].item ){
                setTimeout( () => { this.list[ index ].tween.play(); }, 500 );
                this.counter.opened++;
                if( this.counter.opened === 9 ){
                    this.win();
                }
            }
        },
...
<style lang="scss" scoped>
    ...
    canvas {
        ...
        width: 100%;
        height: 100%;
        .has-winner-true &, .is-status-started & {
            z-index: 9;
        }
    }
